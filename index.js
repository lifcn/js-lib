...
function createInstance(apiToken) {
  var privateScope = {
    ...
    ajaxFailure: function(xhr) {
      if (xhr.status == 401) {
        throw privateScope.eoneoException('Eoneo: User is not authorized')
      } else {
        throw privateScope.eoneoException('Eoneo: Unsuccessful request')
      }
    },
    ...
    xhrRequest: function(type, endpoint, data) {
      ...
      xhr.onload = privateScope.ajaxSuccess;
      xhr.onerror = privateScope.ajaxFailure;
      xhr.send(data);
    },
